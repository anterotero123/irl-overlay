...
async function createBrandCard() {
    ...
    // Jos kortti on jo olemassa, ei tehdä uutta
    ...
    try {
        ...
        // Haetaan oikea AnteroLive.svg GitHub Pagesista
        const response = await fetch("AnteroLive.svg");

        if (!response.ok) {
            throw new Error(
                "AnteroLive.svg ei löytynyt: HTTP " + response.status
            );
        }

        const svgText = await response.text();

        // Luetaan SVG XML-muodossa
        ...
        const sourcePath =
            sourceSvg.querySelector("path");

        if (!sourcePath) {
            throw new Error(
                "AnteroLive.svg ei sisällä <path>-elementtiä."
            );
        }
        ...
        if (!pathData) {
            throw new Error(
                "AnteroLive.svg:n pathilta puuttuu d-arvo."
            );
        }

        // Otetaan alkuperäisen SVG:n viewBox
        const viewBox =
        ...
        console.error(
            "AnteroLive SVG:n lataus epäonnistui:",
            error
        );
...
async function showBrandEvent() {
    ...
    // Varmistetaan, että AnteroLive.svg on ladattu
    ...
    // Jos SVG:n lataus epäonnistui
    if (!brand) {

        console.error(
            "AnteroLive: brand-card puuttuu."
        );
